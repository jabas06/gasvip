  ...
  angular
    ...
    .factory("ratingsService", function($log, $q, $firebaseRef, appConfig) {
      ...
      function getRatingsByStation(stationId) {
        return $firebaseRef.ratingsByStation
          .child(stationId)
          .orderByChild("time")
          .limitToLast(100)
          .once("value")
      ...
      function newRatingForStation(station, user) {
        return $firebaseRef.default
          .child(getTodayRatingPath(user.uid))
          .once("value")
          .then(function(snapshot) {
            ...
            if (ratingNumber > appConfig.MAX_RATINGS_BY_USER) {
              return {
                ...
                message:
                  "Ya has calificado " +
                  appConfig.MAX_RATINGS_BY_USER +
                  " gasolineras el día de hoy. Intenta mañana :)"
              };
            } else if (alreadyRatedStation === true) {
              return {
                canRateStation: false,
                message:
                  "Ya has calificado a esta gasolinera el día de hoy. Intenta mañana :)"
              };
            } else {
              return {
                canRateStation: true,
      ...
      function saveRating(rating, ratingNumber, user) {
        ...
        ratingFanout[
          getTodayRatingPath(user.uid) + "/" + ratingNumber
        ] = rating;
        ratingFanout[
          "ratingsByStation/" + rating.stationId + "/" + rating.uid
        ] = rating;
        ratingFanout[
          "ratingsByUser/" + rating.userId + "/" + rating.uid
        ] = angular.copy(rating);
        ratingFanout[
          "ratingsByUser/" + rating.userId + "/" + rating.uid
        ].avatar = null;
        ...
        $firebaseRef.default
          .update(ratingFanout)
          .then(function() {
            $firebaseRef.stations
              .child(rating.stationId + "/private/rating")
